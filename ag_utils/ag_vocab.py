...
from ag_utils import ag_constant
...
class Vocab(object):
    def __init__(self, filename, load=False, word_counter=None, threshold=0):
        if load:
            ...
            self.id2word, self.word2id = self.load(filename) #调用下面的load函数
    ...
    def save(self, filename): #
        if os.path.exists(filename):
            print("Model vocab file saved at " + filename)
            os.remove(filename)
    ...
    def map(self, token_list):
        ...
        """
        return [self.word2id[w] if w in self.word2id else ag_constant.VOCAB_UNK_ID for w in token_list]
    ...
    def get_embeddings(self, word_vectors=None, dim=100):
        ...
        self.embeddings = 2 * ag_constant.EMB_INIT_RANGE * np.random.rand(self.size, dim) - ag_constant.EMB_INIT_RANGE
