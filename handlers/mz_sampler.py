...
class Sampler(object):
    ...
    def get_train_instances_char_man(self, interactions: interactions.ClassificationInteractions,
        ...
        query_contents = interactions.np_query_contents.astype(np.int64)
        query_contents1 = interactions.np_query_contents1.astype(np.int64)
        ...
        query_aggcn_adj = interactions.np_query_aggcn_adj
        ...
        evd_docs_contents1 = np.zeros((query_ids.shape[0], fixed_num_evidences, interactions.padded_doc_length),
                                     np.int64)
        ...
        docs_aggcn_adj = np.zeros((query_ids.shape[0], fixed_num_evidences, interactions.padded_doc_length,
                                 interactions.padded_doc_length), np.float)######加的
        ...
        for i, u in enumerate(query_ids):
            ...
            for idx, (doc_id, doc_label, doc_content, doc_len, docs_adj, doc_aggcn_adj, doc_contents1) in enumerate(zip(*evidences_info)):
                ...
                docs_aggcn_adj[i][idx] = interactions.dict_doc_aggcn_adj[doc_id]
                evd_docs_contents1[i][idx] = doc_contents1
              
        
        return query_ids, query_contents,query_contents1, query_lengths, query_sources, query_char_source, query_adj,query_aggcn_adj,\
               evd_docs_ids, evd_docs_contents, evd_docs_contents1, evd_docs_lens, evd_sources, evd_cnt_each_query, \
               evd_docs_char_source_contents, query_labels, evd_docs_adj, docs_aggcn_adj
