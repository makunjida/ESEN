...
class CharManFitterQueryRepr1(MultiLevelAttentionCompositeFitter):
    ...
    def __init__(self, net: base_model.BaseModel,
        ...
        self.output_size = kargs["output_size"]#=2

    def fit(self,                                                           #用于训练一个机器学习模型
        ...
        for epoch_num in range(self._n_iter):
            ...
            query_ids, left_contents, left_contents1, left_lengths, query_sources, query_char_sources, query_adj, query_aggcn_adj, \
            evd_docs_ids, evd_docs_contents,evd_docs_contents1, evd_docs_lens, evd_sources, evd_cnt_each_query, evd_char_sources, \
            pair_labels, evd_docs_adj, docs_aggcn_adj = self._sampler.get_train_instances_char_man(train_iteractions,
                                                                                   self.fixed_num_evidences)

          
            queries, query_content, query_content1, query_lengths, query_sources, query_char_sources, query_adj, query_aggcn_adj, \
            evd_docs, evd_docs_contents, evd_docs_contents1, evd_docs_lens, evd_sources, evd_cnt_each_query, evd_char_sources, \
            pair_labels, evd_docs_adj, docs_aggcn_adj = my_utils.shuffle(query_ids, left_contents, left_contents1,left_lengths, query_sources,
                                                         query_char_sources, query_adj, query_aggcn_adj, evd_docs_ids, evd_docs_contents,evd_docs_contents1,
                                                         evd_docs_lens, evd_sources, evd_cnt_each_query,
                                                         evd_char_sources, pair_labels, evd_docs_adj, docs_aggcn_adj)#
            epoch_loss, total_pairs = 0.0, 0
            t1 = time.time()
            for (minibatch_num,
                 (batch_query, batch_query_content, batch_query_content1, batch_query_len, batch_query_sources, batch_query_chr_src,
                  batch_query_adj, batch_query_aggcn_adj, batch_evd_docs, batch_evd_contents, batch_evd_contents1, batch_evd_lens, batch_evd_sources,
                  # i.e. claim source
                  batch_evd_cnt_each_query, batch_evd_chr_src, batch_labels, batch_evd_docs_adj, batch_evd_docs_aggcn_adj)) \
                    in enumerate(my_utils.minibatch(queries, query_content, query_content1, query_lengths, query_sources,
                                                    query_char_sources, query_adj, query_aggcn_adj,
                                                    evd_docs, evd_docs_contents, evd_docs_contents1, evd_docs_lens, evd_sources,
                                                    evd_cnt_each_query, evd_char_sources, pair_labels, evd_docs_adj, docs_aggcn_adj,
                ...
                batch_query_content1 = my_utils.gpu(torch.from_numpy(batch_query_content1), self._use_cuda)###
                ...
                batch_query_aggcn_adj = my_utils.gpu(torch.from_numpy(batch_query_aggcn_adj), self._use_cuda)###
                ...
                batch_evd_contents1 = my_utils.gpu(torch.from_numpy(batch_evd_contents1), self._use_cuda)###torch.Size([32, 30, 100])
                ...
                batch_evd_docs_aggcn_adj = my_utils.gpu(torch.from_numpy(batch_evd_docs_aggcn_adj), self._use_cuda)###
                ...
                additional_data = {KeyWordSettings.EvidenceCountPerQuery: batch_evd_cnt_each_query,
                                   ...
                                   KeyWordSettings.Evd_Docs_Aggcn_Adj: batch_evd_docs_adj,
                                   KeyWordSettings.Query_Aggcn_Adj: batch_query_aggcn_adj
                ...
                if self._loss in ["bpr", "hinge", "pce", "bce", "cross_entropy",
                    ...
                    loss = self._get_multiple_evidences_predictions_normal(
                        batch_query, batch_query_content, batch_query_content1, batch_query_len, batch_query_sources,
                        batch_evd_docs, batch_evd_contents,batch_evd_contents1, batch_evd_lens, batch_evd_sources,
            ...
            if verbose:  # validation after each epoch   topN=10
                f1_macro_val = self._output_results_every_epoch(topN, val_interactions, test_interactions,
                                                                         epoch_num, epoch_train_time, epoch_loss)#有evaluate
    ...
    def _get_multiple_evidences_predictions_normal(self, query_ids: torch.Tensor,
        ...
        query_aggcn_adj = kargs[KeyWordSettings.Query_Aggcn_Adj]##
        evd_docs_aggcn_adj = kargs[KeyWordSettings.Evd_Docs_Aggcn_Adj]##
        ...
        e_conts1, q_conts1 ,e_adj1= [], [], []
        e_chr_src_conts = []
        expaned_labels = []

        for evd_cnt, q_cont, q_cont1, q_len, evd_lens, evd_doc_cont, evd_doc_cont1, evd_chr_src, label, evd_adj, evd_aggcn_adj in \
                zip(evd_count_per_query, query_contents, query_contents1, query_lens,
                    evd_docs_lens, evd_doc_contents, evd_doc_contents1, doc_char_source, labels, evd_docs_adj, evd_docs_aggcn_adj):
        ...
        additional_paramters = {
            ...
            KeyWordSettings.Evd_Docs_Adj: e_adj ,                       
            KeyWordSettings.DocContentNoPaddingEvidence1: e_conts1,
            KeyWordSettings.QueryContentNoPaddingEvidence1: q_conts1, 
            KeyWordSettings.Query_Aggcn_Adj: query_aggcn_adj,
            KeyWordSettings.Evd_Docs_Aggcn_Adj: e_adj1
    ...
    def evaluate(self, testRatings: interactions.ClassificationInteractions, K: int, output_ranking=False, **kargs):
        ...
        for query, evidences_info in testRatings.dict_claims_and_evidences_test.items():#
            ...
            evd_ids, labels, evd_contents, evd_lengths, evd_adj ,evd_aggcn_adj, evd_contents1= evidences_info#
            ...
            claim_content1 = testRatings.dict_claim_contents1[query]
            ...
            claim_content1 = np.tile(claim_content1, (1, 1))
            ...
            evd_contents1 = np.array(evd_contents1).reshape(1, len(labels), -1)  # s
            padded_evd_contents1 = self._pad_evidences(evd_contents1)#

            claim_content = my_utils.gpu(my_utils.numpy2tensor(claim_content, dtype=torch.int), self._use_cuda)#
            claim_content1 = my_utils.gpu(my_utils.numpy2tensor(claim_content1, dtype=torch.int), self._use_cuda)
            ...
            evd_contents1 = my_utils.gpu(my_utils.numpy2tensor(evd_contents1, dtype=torch.int),
                                        self._use_cuda)  # (1, x, R)
            padded_evd_contents1 = my_utils.gpu(my_utils.numpy2tensor(padded_evd_contents1, dtype=torch.int),
                                               self._use_cuda)  # (1, x, R)
            # for evidences
            ...
            query_aggcn_adj = np.array([testRatings.dict_query_aggcn_adj[query]])           # (1, L, L)
            query_aggcn_adj = my_utils.gpu(torch.from_numpy(query_aggcn_adj), self._use_cuda)
            ...
            evd_aggcn_adj = np.array([testRatings.dict_doc_aggcn_adj[e] for e in evd_ids])               # (n, R, R)
            evd_aggcn_adj =  my_utils.gpu(torch.from_numpy(evd_aggcn_adj), self._use_cuda)
            ...
            additional_information = {
                ...
                KeyWordSettings.DocContentNoPaddingEvidence1: evd_contents1.view(1 * len(labels), -1),  # (B1, R)
                ...
                KeyWordSettings.QueryContentNoPaddingEvidence1: claim_content1.expand(len(labels), L),
                ...
                KeyWordSettings.Evd_Docs_Aggcn_Adj: evd_aggcn_adj,
                KeyWordSettings.Query_Aggcn_Adj: query_aggcn_adj
            }
    ...
    def _computing_metrics(self, true_labels: List[int], predicted_labels: List[float], predicted_probs: List[float]):
        ...
        results[KeyWordSettings.Accuracy] = accuracy
    ...
    def _output_results_every_epoch(self, topN: int, val_interactions: interactions.ClassificationInteractions,
        ...
        accuracy_val = result_val[KeyWordSettings.Accuracy]
        ...
        if epoch_num and epoch_num % self._testing_epochs == 0:
            ...
            accuracy_test = result_test[KeyWordSettings.Accuracy]
            ...
            TensorboardWrapper.mywriter().add_scalar("accuracy", accuracy_test, epoch_num)
            # TensorboardWrapper.mywriter().add_scalar("ndcg/ndcg_test", ndcg_test, epoch_num)
            self.output_handler.myprint('|Epoch %03d | Test F1_macro = %.5f  | Accuracy: %.5f | Testing time: %04.1f(s)'
                                        % (epoch_num, f1_macro_test, accuracy_test, testing_time))
        ...
        self.output_handler.myprint('|Epoch %03d | Train time: %04.1f(s) | Train loss: %.3f'
                                    ...
                                    '| Val F1_micro = %.3f | Val Accuracy = %.3f | Validation time: %04.1f(s)'
                                    % (epoch_num, epoch_train_time, epoch_loss, f1_macro_val, auc_val,
                                       f1_micro_val, accuracy_val, valiation_time))
        return f1_macro_val
    ...
    def load_best_model(self, val_interactions: interactions.ClassificationInteractions,
        ...
        self.output_handler.myprint('Best Vad F1_macro = %.5f | Best Vad AUC = %.5f'
                                    ...
                                    % (f1_macro_val, auc_val, f1_macro, f1_micro, auc_test,
                                       ...
                                       precision_mixed_class, recall_mixed_class, f1_mixed_class, accuracy_test))
